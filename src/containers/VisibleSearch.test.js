...
describe('VisibleSearch --- REACT-REDUX (Mount + wrapping in <Provider>)', () => {
    ...
    test('check props matches with initial state', () => {
        ...
        expect(container.find(Search).prop("loading")).toBe(initialState.search.searchLoading);
        expect(container.find(Search).prop("searchResults")).toBe(initialState.search.searchResults);
        expect(container.find(Search).prop("searchInputValue")).toBe(initialState.search.searchInputValue);
    });
