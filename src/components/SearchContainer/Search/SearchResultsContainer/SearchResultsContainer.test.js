...
describe('SearchResultsContainer --- REACT-REDUX (Mount + wrapping in <Provider>)', () => {
	const initialState = {
		search: {
			searchResults: {},
		},
	...
	beforeEach(() => {
		...
		container = mount(
			<Provider store={store}>
				<SearchResultsContainer store={store} />
			</Provider>
		);
	});
	...
	test('check props matches with initial state', () => {
		expect(container.find(SearchResults).prop('searchResults')).toBe(
