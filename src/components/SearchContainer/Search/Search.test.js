...
describe('Search --- Shallow render component', () => {
	...
	test('search contains search text', () => {
		expect(component.find('input[name="searchInput"]').prop('value')).toEqual(
			searchText
		);
	});

	test('contain search results container', () => {
		expect(component.find(SearchResultsContainer).length).toBe(1);
	});
	...
	test("dont show search result when it's empty", () => {
		...
		expect(
			component.find('input[name="searchInput"]').hasClass('show-results')
		).toEqual(false);
	});
