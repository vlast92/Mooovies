...
describe('ACTIONS --- Test action creators', () => {
    ...
    test('actionCreator searchInputChange', () => {
        ...
        const searchInputValue = "Blade", action = searchInputChange(searchInputValue);
    ...
    test('actionCreator receiveSearchResult', () => {
        ...
        const searchResults = { Search: {}, Response: 'True' }, action = receiveSearchResult(searchResults);
        expect(action).toEqual({type: RECEIVE_SEARCH_RESULTS, searchResults})
    });

    test('actionCreator makingSearchRequest', () => {

        const searchWords = "Blade runner", action = makingSearchRequest(searchWords);
        expect(action).toEqual({type: MAKING_SEARCH_REQUEST, searchWords})
    });

    test('actionCreator searchingRequestFailed', () => {

        const error = "Movie not found.", action = searchingRequestFailed(error);
    ...
    test('actionCreator makingGetMovieRequest', () => {
        ...
        const imdbID = "tt0083658", action = makingGetMovieRequest(imdbID);
    ...
    test('actionCreator getMovieRequestFailed', () => {
        ...
        const error = "Movie not found.", action = getMovieRequestFailed(error);
